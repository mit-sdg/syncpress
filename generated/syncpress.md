...
```reaction
when RequestBoundary.request (attempt, job, path: "/site/advance", requestId)
then
  Phasing.advance (attempt, job)
```

### fullSite.AdvanceSiteBuild#2

```reaction
when Phasing.advance (attempt, job, result.attempt: nextAttempt, phase, transitioned), asked by fullSite.AdvanceSiteBuild
where
  earlier, RequestBoundary.request (attempt, job, path: "/site/advance", requestId)
then
  RequestBoundary.respond (attempt: nextAttempt, phase, requestId, transitioned)
```
...
### fullSite.StartSiteBuild:already-started

```reaction
when RequestBoundary.request (path: "/site/start", requestId, sequence)
where
  Phasing._latest (sequence)
then
  RequestBoundary.respond (error: "BUILD_ALREADY_STARTED", requestId)
```

### fullSite.StartSiteBuild:start

```reaction
when RequestBoundary.request (path: "/site/start", requestId, sequence)
where
  no Phasing._latest (sequence)
then
  Phasing.start (sequence)
```

### fullSite.StartSiteBuild:start#2

```reaction
when Phasing.start (sequence, attempt, job, phase), asked by fullSite.StartSiteBuild:start
where
  earlier, RequestBoundary.request (path: "/site/start", requestId, sequence)
then
  RequestBoundary.respond (attempt, job, phase, requestId)
```
...
- `/site/advance` — requires `attempt`, `job`
