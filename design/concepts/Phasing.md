...
`Name = Text` Phase = Text

`Phases = List<Phase>` An ordinary dense phase plan.

`State = "running" | "finished" | "failed"`

`PhaseAttempt = identity` The opaque identity of one announced phase of one Job.

Text is a well-formed Unicode string. A phase plan is an ordinary dense list of Text values with no extra properties. A sequence has at least one phase, and a phase occurs at most once in its sequence.

Sequence and Job values are opaque identities. A Sequence identity is a deterministic encoding of its Name and survives redeclaration. A PhaseAttempt is a deterministic encoding of its Job and phase index. The result of `completePhase` contains either the next Phase and its PhaseAttempt or `null` for both. `null` means that the Job has finished and cannot trigger phase work.
...
Rule: sequence-name: No two Sequences have the same Name.
```
...
declare(name: Name, phases: Phases) : return (sequence: Sequence, changed: Flag)
  ...
  then
    produce that sequence and changed false
...
completePhase(job: Job, attempt: PhaseAttempt) : return (job: Job, name: Name, phase: Phase | null, attempt: PhaseAttempt | null, transitioned: Flag)
  ...
  then
    produce its recorded next phase and attempt with transitioned false
    return job, name, phase, attempt, transitioned
  ...
  then
    ...
    produce job, next phase and attempt, and transitioned true
    return job, name, phase, attempt, transitioned
