...
A realization may memoize derived bytes across instances. Reuse is scoped to
source content, requested width and format, the rendering recipe, and codec
versions. Cached bytes must pass the same format, dimensions, full decoding,
and animation checks as newly generated bytes. Missing, corrupt, or inaccessible
cache entries are regenerated; cache write failures do not fail generation.
The cache does not replace source validation or retain Originals or Renditions
as concept state. The exact original fallback remains a copy of admitted bytes.
Independent renditions may be produced concurrently by a bounded worker pool.
Completion order does not change query ordering. After a failure, generation
waits for already-started work to settle before refusing, without committing a
