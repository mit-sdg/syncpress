...
`Values = null | Flag | Number | JavaScriptString | List<Values> | Mapping` Mapping = Map<JavaScriptString, Values> Keys are JavaScript strings; no key order is implied. Policy = Mapping Path = Text Name = JavaScriptString Address = Text Origin = Text Direction = "asc" | "desc" Field = Text Condition = Mapping Code = "INVALID_CONFIGURATION"

`Problem = record` code: Code message: Text line: Number column: Number

Configured default and collection matches must be portable globs. Collection sort fields and conditions must satisfy the catalog field and condition contracts. These failures are policy problems at their YAML locations. An accepted `site.origin` with a trailing slash is stored without that slash.
...
assess(source: Text) : return (policy: Policy, sources: Values)
  ...
  then
    produce a copy of its policy and the content, templates, and public source plan
    return policy, sources
```
