...
test("its principle: attempts retain selected policy and advance each stage once", () => {
  ...
  expect(rendering.settleBody({ rendering: markdownAttempt.rendering })).toMatchObject({ transitioned: true });
  expect(rendering._active({ rendering: markdownAttempt.rendering })[0]?.stage).toBe("body-settled");
  expect(rendering.settleBody({ rendering: markdownAttempt.rendering })).toMatchObject({ transitioned: false });
  expect(rendering.settleLayout({ rendering: markdownAttempt.rendering })).toMatchObject({ transitioned: true });
  expect(rendering._active({ rendering: markdownAttempt.rendering })).toEqual([]);
  expect(rendering.settleLayout({ rendering: markdownAttempt.rendering })).toMatchObject({ transitioned: false });
...
test("registry refusals, promises, and assembled outcomes match the specification", async () => {
  ...
  expect(await app.concepts.Rendering.begin({ subject: "page", path: "page.txt", profile: 1, template: "page.html", ...attempts })).toEqual({
    error: "INVALID_TEXT",
    detail: "Rendering subjects, paths, profile names, template names, and failure reasons must be well-formed text.",
  });
  const attempt = (await app.concepts.Rendering.begin({ subject: "page", path: "page.md", ...markdown, ...attempts })) as {
    rendering: string;
  };
