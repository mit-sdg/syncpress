...
const { DeliveryArbitration, Diagnosing, Emitting, Filing, Governing, Locating, Phasing } = conceptRefs;
...
export const StartedSiteBuildsBeginAggregateDelivery = reaction(({ job }) =>
  when(Phasing.start({}).responds({ job, name: PHASE_SEQUENCE, phase: "locate" }))
    .then(DeliveryArbitration.beginDelivery({ task: job })),
);

export const BegunSiteBuildDeliveriesRetractStagingDiagnostics = reaction(({ job }) =>
  when(DeliveryArbitration.beginDelivery({ task: job }).responds({}))
    .where(earlier(Phasing.start, {}, { job, name: PHASE_SEQUENCE, phase: "locate" }))
    .then(Diagnosing.retractGroup({ scope: DIAGNOSTIC_SCOPES.staging, source: CONFIGURATION_PATH })),
);

export const LocateGroundsSiteDirectory = reaction(({ path }) =>
  when(Diagnosing.retractGroup({ scope: DIAGNOSTIC_SCOPES.staging, source: CONFIGURATION_PATH }).responds({}))
    .where(
      earlier(Phasing.start, {}, { name: PHASE_SEQUENCE, phase: "locate" }),
      Locating._requested({ name: PLACES.base }).is({ path }),
    )
    .then(Locating.establishBase({ path })),
);

export const UngroundableSiteDirectoryDiagnoses = reaction(({ path, code, detail }) =>
  when(Locating.establishBase({ path }).responds({ status: "problem", code, detail }))
    .then(Diagnosing.report({ ...staged, code, message: detail })),
);

export const GroundedSiteAdmitsConfiguration = reaction(() =>
  when(Locating.establishBase({}).responds({ status: "grounded" }))
    .then(Locating.inspectLocation({ name: PLACES.settings, path: CONFIGURATION_PATH })),
);

export const AdmittedConfigurationIsLoaded = reaction(({ path }) =>
  when(Locating.inspectLocation({ name: PLACES.settings }).responds({ status: "admitted", path }))
    .then(Filing.replaceTreeFromFile({ name: ROOTS.project, source: path, path: CONFIGURATION_PATH })),
);

export const UnreadableConfigurationDiagnoses = reaction(({ code, detail }) =>
  when(Filing.replaceTreeFromFile({ name: ROOTS.project, path: CONFIGURATION_PATH }).responds({ status: "problem", code, detail }))
    .then(Diagnosing.report({ ...staged, code, message: detail })),
);

export const LoadedConfigurationIsAssessed = reaction(({ root, file, text }) =>
  when(Filing.replaceTreeFromFile({ name: ROOTS.project, path: CONFIGURATION_PATH }).responds({ status: "loaded", root, file }))
    .where(
...
export const StageAdmitsSourceRoots = reaction(({ root, directory }) =>
  when(Phasing.completePhase({}).responds({ name: PHASE_SEQUENCE, phase: "stage", transitioned: true }))
    .where(Governing._sources({}).is({ name: root, path: directory }))
    .then(Locating.inspectLocation({ name: root, path: directory })),
);
...
export const StageAdmitsRequestedDestination = reaction(({ directory }) =>
  when(Phasing.completePhase({}).responds({ name: PHASE_SEQUENCE, phase: "stage", transitioned: true }))
    ...
    .then(Locating.inspectLocation({ name: PLACES.destination, path: directory })),
);
...
export const StageAdmitsConfiguredOutput = reaction(({ directory }) =>
  when(Phasing.completePhase({}).responds({ name: PHASE_SEQUENCE, phase: "stage", transitioned: true }))
    ...
    .then(Locating.inspectLocation({ name: PLACES.output, path: directory })),
);

export const UnresolvableLocationDiagnoses = reaction(({ name, path, code, detail }) =>
  when(Locating.inspectLocation({ name, path }).responds({ status: "problem", code, detail }))
    .then(Diagnosing.report({ ...staged, code, message: detail })),
);

export const AdmittedSourceRootsAreLoaded = reaction(({ root, directory, real }) =>
  when(Locating.inspectLocation({ name: root, path: directory }).responds({ status: "admitted", real, contained: true, resolved: true }))
    .where(Governing._sources({}).is({ name: root, path: directory }))
    .then(Filing.replaceTreeFromDirectory({ name: root, directory: real })),
);

export const UnloadableSourceRootDiagnoses = reaction(({ root, code, detail }) =>
  when(Filing.replaceTreeFromDirectory({ name: root }).responds({ status: "problem", code, detail }))
...
export const ConfiguredOutputDirectsPublication = reaction(({ directory, real, prefix }) =>
  when(Locating.inspectLocation({ name: PLACES.output, path: directory }).responds({ status: "admitted", real, contained: true, resolved: true }))
    .where(PublicationTransactionPrefix({ destination: real }).is({ prefix }))
    .then(Emitting.configureDestination({ destination: real, prefix })),
);

export const DestinationDirectsPublication = reaction(({ directory, real, prefix }) =>
  when(Locating.inspectLocation({ name: PLACES.destination, path: directory }).responds({ status: "admitted", real }))
    .where(PublicationTransactionPrefix({ destination: real }).is({ prefix }))
    .then(Emitting.configureDestination({ destination: real, prefix })),
);

export const UndirectablePublicationDiagnoses = reaction(({ destination, error, detail }) =>
  when(Emitting.configureDestination({ destination }).refuses({ error, detail }))
...
const escapesSite = (place: string, key: string, code: string) =>
  reaction(({ directory, admitted }) =>
    when(Locating.inspectLocation({ name: place, path: directory }).responds({ status: "admitted", place: admitted }))
...
export const OutputOverlappingSourceRootDiagnoses = reaction(({ publication, root, source }) =>
  when(Phasing.completePhase({}).responds({ name: PHASE_SEQUENCE, phase: "settings", transitioned: true }))
    .where(
