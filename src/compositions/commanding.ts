...
export const InterpretCommandLine = endpoint(
  ...
  ({ supplied, words }) =>
    ...
    .then(Commanding.captureArguments({ arguments: supplied }).responds({ words }))
      .then(
...
export const AnnounceUsage = endpoint("/cli/usage", ({ text }) =>
  receive({})
    ...
    .then(Commanding.writeLine({ stream: "output", text }).responds({}))
    .then(respond({})),
);
...
export const AnnounceMisuse = endpoint("/cli/misuse", ({ text }) =>
  receive({})
    ...
    .then(Commanding.writeLine({ stream: "error", text }).responds({}))
    .then(respond({})),
);

export const WriteCommandLine = endpoint("/cli/write", ({ stream, text }) =>
  receive({ stream, text })
    .then(Commanding.writeLine({ stream, text }).responds({}))
    .then(respond({})),
);

export const HoldUntilStopped = endpoint("/cli/hold", ({ reason }) =>
  receive({})
    .then(Holding.awaitStop({}).responds({ reason }))
    .then(respond({ reason })),
);

export const SetCommandLineExit = endpoint("/cli/exit", ({ code }) =>
  receive({ code })
    .then(Commanding.setExitStatus({ code }).responds({}))
    .then(respond({})),
);
